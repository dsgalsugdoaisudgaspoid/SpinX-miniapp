<template>
  <view class="home">
    ...
    <view class="topbg" :style="{ paddingTop: statusBar + 'px' }">
      ...
      <view class="searchrow">
        ...
        <view class="city" @tap="pickCity">
          <text class="cdot"></text><text class="cname">{{ city }}</text
          ><text v-if="multiCity" class="caret">▾</text>
        </view>
      </view>
      <view class="slogan">
    ...
    <view class="notyet" v-if="showNotYet">
      ...
      <view class="nybtns">
        <view class="nybtn ghost" v-if="nearest" @tap="switchToNearest"
          >切换到{{ nearest.name }}</view
        >
    ...
    <scroll-view scroll-y class="scroll">
      ...
      <view class="coachbar" v-if="showCoach">
        <view class="cbcard find" @tap="goCoach">
          ...
          <view class="cbtxt"
            ><text class="cbt">找陪练</text
            ><text class="cbs">认证陪练 · 按等级付费</text></view
          >
        </view>
        <view class="cbcard be" @tap="goCoachApply">
          <text class="cbi">⭐</text>
          <view class="cbtxt"
            ><text class="cbt">成为陪练</text
            ><text class="cbs">考核定级 · 接单赚钱</text></view
          >
        </view>
      </view>
      ...
      <view class="strip">
        <view class="mini" @tap="goRanking">
          <view class="mt"
            ><text>本月骑行榜</text><text class="mono em">TOP</text></view
          >
          <view class="rankrow">
            <view
              v-for="(r, i) in ranks"
        ...
        <view class="mini" @tap="goPoints">
          <view class="mt"
            ><text>月度任务</text
            ><text class="mono em"
              >{{ mission.progress }}/{{ mission.total }}</text
            ></view
          >
          <view class="bar"
            ><view class="barfill" :style="{ width: missionPct + '%' }"></view
          ></view>
      ...
      <view class="memmap" @tap="goMemoryMap">
        ...
        <view class="ethosb"
          ><text class="ethosk">城市记忆</text
          ><text class="ethost ellipsis"
            >SpinX在{{ city }}点亮过的地方，都在这张图上</text
          ></view
        >
      ...
      <view class="ethos" @tap="goPhilosophy">
        <text class="ethosic">🏝</text>
        <view class="ethosb"
          ><text class="ethosk">俱乐部理念</text
          ><text class="ethost ellipsis"
            >和小伙伴一起，去发现世界更多未知的美好</text
          ></view
        >
        <text class="ethosarw">›</text>
      </view>
      ...
      <view class="feedentry" @tap="goFeed">
        ...
        <view class="feb"
          ><text class="fek">动态广场</text
          ><text class="fet">看看大家的骑行故事，也发一条</text></view
        >
        <text class="fearw">›</text>
      </view>

      <!-- 找同学入口：原先只藏在动态广场里（主页→动态广场→找同学，三层才够得到），
           学号是会员身份的核心，主页要能直接进 -->
      <view class="findmate" @tap="goFindMember">
        <text class="ethosic">🔍</text>
        <view class="ethosb"
          ><text class="ethosk">找同学</text
          ><text class="ethost ellipsis"
            >输入 5 位学号，找到 TA 的主页</text
          ></view
        >
        <text class="ethosarw">›</text>
      </view>
      ...
      <scroll-view scroll-x class="chips" show-scrollbar="false">
        <view
          v-for="t in tags"
          :key="t.key"
          :class="['chip', activeTag === t.key ? 'on' : '']"
          @tap="switchTag(t.key)"
        >
          <text v-if="t.icon" class="cic">{{ t.icon }}</text
          >{{ t.name }}
        </view>
      </scroll-view>
      ...
      <view class="sortbar">
        <view class="sl"
          ><text class="s1">近期发车</text
          ><text class="s2">· 环星带你上路</text></view
        >
      ...
      <view class="acts">
        <view
          ...
          :key="a.activityId"
          class="act"
          @tap="goDetail(a.activityId)"
        >
          <view class="poster">
            <text class="ptag">{{ (a.tags && a.tags[0]) || "骑行" }}</text>
            <text class="pseat"
              ><text class="mono">{{ a.currentParticipants }}</text
              >/{{ a.maxParticipants }} 已上车</text
            >
          </view>
          <view class="info">
            <view class="drow">
              ...
              <text class="dsm"
                >{{ fmt(a.startTime).weekday }} ·
                {{ fmt(a.startTime).time }}</text
              >
            </view>
            <text class="atitle ellipsis-2">{{ a.title }}</text>
            <view class="loc">
              <text>📍</text
              ><text class="lname ellipsis flex1">{{ a.meetingPoint }}</text>
            </view>
            <view class="metas">
              <text class="pill">{{ a.distance }}km</text>
              <text class="pill">难度 {{ starStr(a.difficulty) }}</text>
              <text :class="['pill', a.fee > 0 ? 'free' : 'free']">{{
                feeStr(a.fee)
              }}</text>
            </view>
            <view class="lead">
              <view class="lav"></view>
              <text>领队 {{ a.leader ? spx(a.leader.nickname) : "—" }}</text>
            </view>
          </view>
        </view>
...
export default {
  ...
  computed: {
    ...
    sortLabel() {
      return this.sort === "time" ? "最近开始" : "最热门";
    },
    ...
    showNotYet() {
      return this.outOfService && !this.notYetDismissed;
    },
  },
  onLoad() {
  ...
  onPullDownRefresh() {
    Promise.all([this.loadHome(), this.loadList()]).finally(() =>
  ...
  methods: {
    ...
    gradAt(i) {
      const g = [
        "linear-gradient(135deg,#ffd36e,#ff8f6e)",
        "linear-gradient(135deg,#8fd3ff,#5e8bff)",
        "linear-gradient(135deg,#b6f0c9,#5ecb8f)",
      ];
      return g[i % g.length];
    },
    async loadHome() {
    ...
    pickCity() {
      if (!this.multiCity) {
        uni.showToast({
          title: `SpinX 现已扎根${this.city}，更多城市即将上线`,
          icon: "none",
        });
        return;
      }
      ...
      uni.showActionSheet({
        ...
        success: (r) => {
          ...
          if (picked && picked.code !== currentCityCode())
            this.applyCity(picked, "manual");
        },
      });
    },
    ...
    toggleSort() {
      this.sort = this.sort === "time" ? "hot" : "time";
    },
    goDetail(id) {
    ...
    goPhilosophy() {
      uni.navigateTo({ url: "/pages/club/philosophy" });
    },
    goMemoryMap() {
      uni.navigateTo({ url: "/pages/memory/map" });
    },
    goFeed() {
      uni.navigateTo({ url: "/pages/posts/list" });
    },
    goFindMember() {
      uni.navigateTo({ url: "/pages/member/search" });
    },
    async loadActiveRaffle() {
      try {
        this.activeRaffle = await activePublicRaffle();
      } catch (e) {
    ...
    goPublicRaffle() {
      if (this.activeRaffle)
        uni.navigateTo({
          url: "/pages/public-raffle/detail?id=" + this.activeRaffle.raffleId,
        });
    },
    goCreate() {
      uni.navigateTo({ url: "/pages/leader/create" });
    },
    // 打开小程序：若有参与过但没写收获的活动，弹出一句话总结。
    // 每场活动只提醒一次：提醒过的 entryId 落本地存储，跨会话/重启都不再打扰（无论填没填、跳没跳）。
    async checkPendingReview() {
      if (this.reviewShow || !isLoggedIn()) return;
      try {
        const d = await pendingReview();
        const p = d && d.pending;
        if (!p || p.entryId == null) return;
        const reminded = uni.getStorageSync("reviewReminded") || [];
        if (reminded.indexOf(p.entryId) !== -1) return;
        reminded.push(p.entryId);
        if (reminded.length > 200) reminded.splice(0, reminded.length - 200);
        uni.setStorageSync("reviewReminded", reminded);
        this.reviewPending = p;
    ...
    goCoach() {
      uni.navigateTo({ url: "/pages/coach/list" });
    },
    goCoachApply() {
      uni.navigateTo({ url: "/pages/coach/apply" });
    },
    goSearch() {
      uni.switchTab({ url: "/pages/activity/list" });
    },
    goAbout() {
      uni.navigateTo({ url: "/pages/club/about" });
    },
  },
};
</script>
...
.topbg {
  background: linear-gradient(180deg, #dff3e9 0%, #eaf6f1 44%, $paper 100%);
}
...
.brandbar {
  display: flex;
  align-items: center;
  padding: 14rpx 24rpx 4rpx 30rpx;
...
.notyet {
  ...
  border-radius: 22rpx;
  padding: 22rpx 24rpx;
  box-shadow: inset 0 0 0 2rpx #f5e3bd;
}
.nyhd {
  display: flex;
  align-items: flex-start;
...
.hero.a {
  flex: 1.32;
  background: linear-gradient(150deg, #0e1b24, #123a57 60%, #0a5c86);
}
.hero.b {
  flex: 1;
  background: linear-gradient(150deg, #123a2a, #0ba968 90%);
}
.hero .stars {
  position: absolute;
  inset: 0;
  opacity: 0.7;
  background:
    radial-gradient(2rpx 2rpx at 22% 30%, #fff, transparent),
    radial-gradient(2rpx 2rpx at 72% 22%, #cdefff, transparent),
    radial-gradient(2rpx 2rpx at 84% 46%, #fff, transparent);
}
.hero .eye {
  position: absolute;
  top: 22rpx;
...
.cbcard.find {
  background: linear-gradient(120deg, #12d07a, #0ba968);
}
.cbcard.be {
  background: linear-gradient(120deg, #123a57, #0a5c86);
}
.cbi {
  font-size: 44rpx;
}
...
.barfill {
  height: 100%;
  border-radius: 8rpx;
  background: linear-gradient(90deg, $green, $green-deep);
}
.goal {
  display: block;
  font-size: 21rpx;
...
.findmate {
  ...
  padding: 24rpx 26rpx;
  border-radius: 26rpx;
  background: linear-gradient(120deg, $night-1, $night-2 70%, $night-3);
  color: #fff;
}
.memmap {
  background: linear-gradient(120deg, $night-2, $night-3 70%, #0d7ba8);
}
.findmate {
  background: linear-gradient(120deg, $night-1, #1f4d3a 70%, #14663f);
}
.ethosic {
  font-size: 44rpx;
  flex: none;
...
.lav {
  ...
  height: 34rpx;
  border-radius: 50%;
  background: linear-gradient(135deg, #5ecb8f, #0ba968);
}
...
.fab {
  ...
  align-items: center;
  gap: 8rpx;
  background: linear-gradient(120deg, #b5379e, #7a2e86);
  color: #fff;
  padding: 18rpx 28rpx;
  border-radius: 40rpx;
  box-shadow: 0 14rpx 30rpx -8rpx rgba(122, 46, 134, 0.7);
}
.fabi {
  font-size: 34rpx;
}
...
.fabdot {
  ...
  border-radius: 50%;
  background: #ff5a5a;
  box-shadow: 0 0 0 4rpx rgba(255, 90, 90, 0.25);
}
/* 领队发起活动悬浮按钮 */
.fab-add {
  position: fixed;
  ...
  height: 108rpx;
  border-radius: 50%;
  background: linear-gradient(135deg, #12d07a, #0ba968);
  box-shadow: 0 14rpx 30rpx -8rpx rgba(11, 169, 104, 0.7);
  display: flex;
  align-items: center;
  justify-content: center;
}
